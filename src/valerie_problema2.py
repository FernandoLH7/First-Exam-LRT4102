...
class TurtleController:
    ...
    def dibujar_recntangulo(self, x_inicio, y_inicio):
        """Dibujar un recntangulo."""
        rospy.loginfo("Dibujando un recntangulo...")

        # Calculamos las esquinas del recntangulo
        puntos = [
            (x_inicio + 3, y_inicio),
        ...
        print("Coordenadas del recntangulo a dibujar:")
        for i, (x, y) in enumerate(puntos):
            print(f"Esquina {i + 1}: ({x}, {y})")

        # Validamos si el recntangulo cabe en el área de trabajo
        if not self.validar_espacio(puntos):
            print("El recntangulo no cabe en el área de trabajo. Intenta con otras coordenadas.")
            return False

        # Movemos la tortuga a cada esquina del recntangulo
        for x, y in puntos:
            self.mover_a_punto(x, y)
            time.sleep(1)

        print("¡recntangulo terminado!")
        return True

    def dibujar_trapezoide(self, x_inicio, y_inicio):
        """Dibujar un trapezoide."""
        rospy.loginfo("Dibujando un trapezoide...")

        # Calculamos las esquinas del trapezoide
        puntos = [
            (x_inicio + 4, y_inicio),
            (x_inicio + 3, y_inicio + 2),
            (x_inicio + 0.75, y_inicio + 1.75),
            (x_inicio, y_inicio)
        ]

        # Mostramos las coordenadas de las esquinas al usuario
        print("Coordenadas del trapezoide a dibujar:")
        for i, (x, y) in enumerate(puntos):
            print(f"Esquina {i + 1}: ({x}, {y})")

        # Validamos si el trapezoide cabe en el área de trabajo
        if not self.validar_espacio(puntos):
            print("El trapezoide no cabe en el área de trabajo. Intenta con otras coordenadas.")
            return False

        # Movemos la tortuga a cada esquina del trapezoide
        for x, y in puntos:
            self.mover_a_punto(x, y)
            time.sleep(1)

        print("¡trapezoide terminado!")
        return True
    ...
    def menu(self):
        ...
        while not rospy.is_shutdown():
            print("\nControl de Tortugas")
            print("1 -> Dibujar recntangulo")
            print("2 -> Dibujar trapezoide")
            ...
            if opcion == '4':
                ...
                print("Opción seleccionada: Dibujar recntangulo")

                while True:
                    print("\nCoordenadas para el recntangulo")
                    x_recntangulo = float(input("Coordenada x: "))
                    y_recntangulo = float(input("Coordenada y: "))

                    # Mostramos las coordenadas de las esquinas antes de validar
                    puntos = [
                        (x_recntangulo+3, y_recntangulo),
                        (x_recntangulo+3, y_recntangulo+2),
                        (x_recntangulo, y_recntangulo + 2),
                        (x_recntangulo, y_recntangulo)
                    ]
                    print("Coordenadas del recntangulo a dibujar:")
                    for i, (x, y) in enumerate(puntos):
                        print(f"Esquina {i + 1}: ({x}, {y})")
                    ...
                    # Validamos si el recntangulo cabe en el área de trabajo
                    if not self.validar_espacio(puntos):
                        print("El recntangulo no cabe en el área. Intenta con otras coordenadas.")
                        continue

                    self.crear_tortuga(x_recntangulo, y_recntangulo, 'turtle1')
                    self.suscribirse_posicion('turtle1')
                    self.configurar_publicador('turtle1')
                    if self.dibujar_recntangulo(x_recntangulo, y_recntangulo):
                        self.eliminar_tortuga('turtle1')
                        break
                    self.eliminar_tortuga('turtle1')
                ...
                print("Opción seleccionada: Dibujar trapezoide")

                while True:
                    print("\nCoordenadas para el trapezoide")
                    x_trapezoide = float(input("Coordenada x: "))
                    y_trapezoide = float(input("Coordenada y: "))

                    # Mostramos las coordenadas de las esquinas antes de validar
                    puntos = [
                        (x_trapezoide + 4, y_trapezoide),
                        (x_trapezoide + 3, y_trapezoide + 2),
                        (x_trapezoide + 0.75, y_trapezoide + 1.75),
                        (x_trapezoide, y_trapezoide)
                    ]
                    print("Coordenadas del trapezoide a dibujar:")
                    for i, (x, y) in enumerate(puntos):
                        print(f"Esquina {i + 1}: ({x}, {y})")
                    ...
                    # Validamos si el trapezoide cabe en el área de trabajo
                    if not self.validar_espacio(puntos):
                        print("El trapezoide no cabe en el área. Intenta con otras coordenadas.")
                        continue

                    self.crear_tortuga(x_trapezoide, y_trapezoide, 'turtle2')
                    self.suscribirse_posicion('turtle2')
                    self.configurar_publicador('turtle2')
                    if self.dibujar_trapezoide(x_trapezoide, y_trapezoide):
