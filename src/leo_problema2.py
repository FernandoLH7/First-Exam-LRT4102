...
class TurtleController:
    ...
    def dibujar_cuadrado(self, x_inicio, y_inicio):
        ...
        puntos = [
            (x_inicio + 3, y_inicio),
            (x_inicio+3, y_inicio + +3),
            (x_inicio, y_inicio+3),
            (x_inicio, y_inicio)
        ]

        # Mostramos las coordenadas de las esquinas al usuario
        print("Coordenadas del cuadrado a dibujar:")
        for i, (x, y) in enumerate(puntos):
            print(f"Esquina {i + 1}: ({x}, {y})")

        # Validamos si el cuadrado cabe en el área de trabajo
        if not self.validar_espacio(puntos):
            print("El cuadrado no cabe en el área de trabajo. Intenta con otras coordenadas.")
            return False

        # Movemos la tortuga a cada esquina del cuadrado
        for x, y in puntos:
    ...
    def dibujar_romboide(self, x_inicio, y_inicio):
        """Dibujar un romboide."""
        rospy.loginfo("Dibujando un romboide...")

        # Calculamos las esquinas del romboide
        puntos = [
            (x_inicio + 2, y_inicio),
        ...
        # Mostramos las coordenadas de las esquinas al usuario
        print("Coordenadas del romboide a dibujar:")
        for i, (x, y) in enumerate(puntos):
            print(f"Esquina {i + 1}: ({x}, {y})")

        # Validamos si el romboide cabe en el área de trabajo
        if not self.validar_espacio(puntos):
            print("El romboide no cabe en el área de trabajo. Intenta con otras coordenadas.")
            return False

        # Movemos la tortuga a cada esquina del romboide
        for x, y in puntos:
            self.mover_a_punto(x, y)
            time.sleep(1)

        print("¡romboide terminado!")
        return True
        ...
        while not rospy.is_shutdown():
            ...
            if opcion == '4':
                ...
                while True:
                    ...
                    x_cuadrado = float(input("Coordenada x: "))
                    y_cuadrado = float(input("Coordenada y: "))

                    # Mostramos las coordenadas de las esquinas antes de validar
                    puntos = [
                        (x_cuadrado+3, y_cuadrado),
                        (x_cuadrado+3, y_cuadrado+3),
                        (x_cuadrado, y_cuadrado + 3),
                        (x_cuadrado, y_cuadrado)
                    ]
                    print("Coordenadas del cuadrado a dibujar:")
                    for i, (x, y) in enumerate(puntos):
                        print(f"Esquina {i + 1}: ({x}, {y})")
                    ...
                    # Validamos si el cuadrado cabe en el área de trabajo
                    if not self.validar_espacio(puntos):
                        print("El cuadrado no cabe en el área. Intenta con otras coordenadas.")
                        continue

                    self.crear_tortuga(x_cuadrado, y_cuadrado, 'turtle1')
                    self.suscribirse_posicion('turtle1')
                    self.configurar_publicador('turtle1')
                    if self.dibujar_cuadrado(x_cuadrado, y_cuadrado):
                        self.eliminar_tortuga('turtle1')
                        break
                    self.eliminar_tortuga('turtle1')
                ...
                while True:
                    print("\nCoordenadas para el romboide")
                    x_romboide = float(input("Coordenada x: "))
                    y_romboide = float(input("Coordenada y: "))

                    # Mostramos las coordenadas de las esquinas antes de validar
                    puntos = [
                        (x_romboide + 2, y_romboide),
                        (x_romboide + 3, y_romboide + 2),
                        (x_romboide + 1, y_romboide + 2),
                        (x_romboide, y_romboide)
                    ]
                    print("Coordenadas del romboide a dibujar:")
                    for i, (x, y) in enumerate(puntos):
                        print(f"Esquina {i + 1}: ({x}, {y})")
                    ...
                    # Validamos si el romboide cabe en el área de trabajo
                    if not self.validar_espacio(puntos):
                        print("El romboide no cabe en el área. Intenta con otras coordenadas.")
                        continue

                    self.crear_tortuga(x_romboide, y_romboide, 'turtle2')
                    self.suscribirse_posicion('turtle2')
                    self.configurar_publicador('turtle2')
                    if self.dibujar_romboide(x_romboide, y_romboide):
