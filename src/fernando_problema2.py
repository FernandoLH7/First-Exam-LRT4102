...
class TurtleController:
    ...
    def dibujar_rombo(self, x_inicio, y_inicio):
        """Dibujar un rombo."""
        rospy.loginfo("Dibujando un rombo...")

        # Calculamos las esquinas del rombo
        puntos = [
        ...
        print("Coordenadas del rombo a dibujar:")
        for i, (x, y) in enumerate(puntos):
            print(f"Esquina {i + 1}: ({x}, {y})")

        # Validamos si el rombo cabe en el área de trabajo
        if not self.validar_espacio(puntos):
            print("El rombo no cabe en el área de trabajo. Intenta con otras coordenadas.")
            return False

        # Movemos la tortuga a cada esquina del rombo
        for x, y in puntos:
            self.mover_a_punto(x, y)
            time.sleep(1)

        print("¡Rombo terminado!")
        return True

    def dibujar_pentagono(self, x_inicio, y_inicio):
        """Dibujar un pentágono."""
        rospy.loginfo("Dibujando un pentágono...")

        # Calculamos las esquinas del pentágono
        puntos = [
            (x_inicio + 2, y_inicio),
            (x_inicio + 3, y_inicio + 2),
            (x_inicio + 1, y_inicio + 4),
            (x_inicio - 1, y_inicio + 2),
            (x_inicio, y_inicio)
        ]

        # Mostramos las coordenadas de las esquinas al usuario
        print("Coordenadas del pentágono a dibujar:")
        for i, (x, y) in enumerate(puntos):
            print(f"Esquina {i + 1}: ({x}, {y})")

        # Validamos si el pentágono cabe en el área de trabajo
        if not self.validar_espacio(puntos):
            print("El pentágono no cabe en el área de trabajo. Intenta con otras coordenadas.")
            return False

        # Movemos la tortuga a cada esquina del pentágono
        for x, y in puntos:
            self.mover_a_punto(x, y)
            time.sleep(1)

        print("¡Pentágono terminado!")
        return True
    ...
    def menu(self):
        ...
        while not rospy.is_shutdown():
            ...
            if opcion == '4':
                ...
                while True:
                    print("\nCoordenadas para el rombo")
                    x_rombo = float(input("Coordenada x: "))
                    y_rombo = float(input("Coordenada y: "))
                    ...
                    # Validamos si el rombo cabe en el área de trabajo
                    if not self.validar_espacio(puntos):
                        print("El rombo no cabe en el área. Intenta con otras coordenadas.")
                        continue

                    self.crear_tortuga(x_rombo, y_rombo, 'turtle1')
                    self.suscribirse_posicion('turtle1')
                    self.configurar_publicador('turtle1')
                    if self.dibujar_rombo(x_rombo, y_rombo):
                        self.eliminar_tortuga('turtle1')
                        break
                    self.eliminar_tortuga('turtle1')
                ...
                while True:
                    print("\nCoordenadas para el pentágono")
                    x_pentagono = float(input("Coordenada x: "))
                    y_pentagono = float(input("Coordenada y: "))

                    # Mostramos las coordenadas de las esquinas antes de validar
                    puntos = [
                        (x_pentagono + 2, y_pentagono),
                        (x_pentagono + 3, y_pentagono + 2),
                        (x_pentagono + 1, y_pentagono + 4),
                        (x_pentagono - 1, y_pentagono + 2),
                        (x_pentagono, y_pentagono)
                    ]
                    print("Coordenadas del pentágono a dibujar:")
                    for i, (x, y) in enumerate(puntos):
                        print(f"Esquina {i + 1}: ({x}, {y})")
                    ...
                    # Validamos si el pentágono cabe en el área de trabajo
                    if not self.validar_espacio(puntos):
                        print("El pentágono no cabe en el área. Intenta con otras coordenadas.")
                        continue

                    self.crear_tortuga(x_pentagono, y_pentagono, 'turtle2')
                    self.suscribirse_posicion('turtle2')
                    self.configurar_publicador('turtle2')
                    if self.dibujar_pentagono(x_pentagono, y_pentagono):
